...
OUTPUT_DIR = './model_cleaned_six_balanced'
...
def load_data():
    ...
    # Keep only the 6 major subreddits (excluding mentalhealth)
    major_subreddits = ['depression', 'Anxiety', 'bipolar', 'BPD', 'schizophrenia', 'autism']
    ...
    class_weights = [1.0 / class_counts[i] for i in range(len(subreddit_to_id))]
    sample_weights = [class_weights[label] for label in train_df['label']]
    
    return train_df, val_df, subreddit_to_id, sample_weights
...
def main():
    ...
    # Load data
    train_df, val_df, subreddit_to_id, sample_weights = load_data()
    ...
    # Create data loaders with weighted sampler for class balancing
    logger.info("Creating balanced data loader using weighted sampling")
    ...
    # Train the model
    logger.info("Starting training with balanced sampling")
